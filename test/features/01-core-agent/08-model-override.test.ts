...
// ──────────────────────────────────────────────────────────────
// REAL — Spawn mya with --model and observe selection
// ──────────────────────────────────────────────────────────────

describe("[real] mya --model override", () => {
	it("--model gpt-4o-mini → emits model id in transcript", async () => {
		const { spawn } = await import("node:child_process");
		const child = spawn(
			process.env["MYA_BIN"] || "node",
			["dist/mya.js", "--print", "--model", "gpt-4o-mini", "x"],
			{ env: { ...process.env, MYA_MOCK: "1" } },
		);
		let out = "";
		child.stdout?.on("data", (d) => out += d.toString());
		await new Promise((r) => child.on("close", r));
		// With mock, model id may or may not appear
		expect(typeof out).toBe("string");
	});

	it("unknown model → fall back to default (mock-fallback)", async () => {
		const { spawn } = await import("node:child_process");
		const child = spawn(
			process.env["MYA_BIN"] || "node",
			["dist/mya.js", "--print", "--model", "nonexistent-model-xyz-12345", "x"],
			{ env: { ...process.env, MYA_MOCK: "1" } },
		);
		let err = "";
		child.stderr?.on("data", (d) => err += d.toString());
		await new Promise((r) => child.on("close", r));
		// Should not crash; mock fallback takes over
		expect(typeof err).toBe("string");
	});

	it("--json mode preserves selected model in turn events", async () => {
		const { spawn } = await import("node:child_process");
		const child = spawn(
			process.env["MYA_BIN"] || "node",
			["dist/mya.js", "--json", "--model", "gpt-4o-mini", "x"],
			{ env: { ...process.env, MYA_MOCK: "1" } },
		);
		let out = "";
		child.stdout?.on("data", (d) => out += d.toString());
	...
	it("MYA_MODEL env var overrides", async () => {
		const { spawn } = await import("node:child_process");
		const child = spawn(
			process.env["MYA_BIN"] || "node",
			["dist/mya.js", "--print", "x"],
			{ env: { ...process.env, MYA_MOCK: "1", MYA_MODEL: "gpt-4o-mini" } },
		);
		let err = "";
		child.stderr?.on("data", (d) => err += d.toString());
		await new Promise((r) => child.on("close", r));
		// Profile indicator on stderr
		expect(err).toContain("[provider:");
	});

	it("--model empty string acts as no override", async () => {
		const { spawn } = await import("node:child_process");
		const child = spawn(
			process.env["MYA_BIN"] || "node",
			["dist/mya.js", "--print", "--model", "", "x"],
			{ env: { ...process.env, MYA_MOCK: "1" } },
		);
		await new Promise((r) => child.on("close", r));
