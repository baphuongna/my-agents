...
// Vendored packages — copied from upstream, not subject to mya invariants.
const VENDORED = ["intercom"];
...
function walk(dir) {
  for (const entry of readdirSync(dir)) {
    ...
    if (st.isDirectory()) {
      ...
      if (VENDORED.some(v => full.includes(join("packages", v)))) continue;
      walk(full);
    } else if (/\.(m?[tc]sx?)$/.test(entry) && !/\.test\.[tj]sx?$/.test(entry)) {
      if (full.endsWith(join("core", "src", "time.ts"))) continue; // the sole allowed source
      if (full.endsWith(join("web", "src", "lib", "format.ts"))) continue; // browser utility — nowWallclock not available in browser
      const text = readFileSync(full, "utf8");
      for (const line of text.split("\n")) {
        const trimmed = line.trim();
        if (trimmed.startsWith("//") || trimmed.startsWith("*") || trimmed.startsWith("/*")) continue; // skip comments
        // Invariant #10: ban Date.now() as a wallclock source.
        // .getTime() on Date objects and performance.now() for profiling are allowed —
        // the invariant targets direct clock access (Date.now(), new Date()).
        if (/Date\.now\s*\(|Date\[(['"]?)now/.test(line)) offenders.push(`${full}: ${trimmed}`);
      }
    }
  }
}
