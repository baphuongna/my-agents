...
| 2 | Never cache allowlists/`denied_tools` in handles | 🟡 | permission resolves via `TurnContext` per-call (`packages/tools/src/permission.ts`). **Gap:** no ESLint rule banning allowlist fields on handle classes (ESLint is TS-7-incompatible — enforced by the Date.now grep guard pattern instead). |
...
| 10 | Never duplicate the time helper | ✅ | `core.time` (TS) + `natives.now_*` (Rust) are the sole sources; all 39 `Date.now()` sites replaced with `nowWallclock()`; enforced by BOTH a vitest guard test (`invariant-time.test.ts`) AND `npm run lint` (scripts/lint.mjs grep guard). |
...
| 19 | Transports depend on `core` only; no cross-transport imports | ✅ | all 4 transports (`tui`/`rpc`/`sdk`/`print`) depend on `core` only; none import each other or `child_process`. Enforced by `npm run lint:deps` (madge --circular, clean). |
