...
describe("[unit] osv-check tool", () => {
  ...
  beforeEach(() => { originalFetch = global.fetch; }, {} as never);
  afterEach(() => { global.fetch = originalFetch; }, {} as never);
  ...
  it("missing package/version → error", async () => {
    const r = await osvCheckTool.run({ package: "lodash" }, {} as never);
  ...
  it("no vulnerabilities → vulnerable=false", async () => {
    ...
    const r = await osvCheckTool.run({ package: "safe-pkg", version: "1.0.0" }, {} as never);
  ...
  it("vulnerabilities found → vulnerable=true + details", async () => {
    ...
    const r = await osvCheckTool.run({ package: "lodash", version: "4.17.20" }, {} as never);
    expect(r.ok).toBe(true);
  ...
  it("API error → tool error", async () => {
    ...
    const r = await osvCheckTool.run({ package: "x", version: "1.0" }, {} as never);
    expect(r.ok).toBe(false);
  ...
  it("default ecosystem is npm", async () => {
    ...
    await osvCheckTool.run({ package: "pkg", version: "1.0" }, {} as never);
