...
// mya fork: cookie-based auth (no session token header needed).
// The gateway sets an HttpOnly mya_ws cookie on GET /.
// All fetch calls carry it via credentials: "include".
const SESSION_HEADER = "X-Hermes-Session-Token";

// mya fork: map Hermes /api/* paths to mya gateway endpoints
function mapMyaUrl(url: string): string {
  // Exact + prefix mappings from Hermes API → mya gateway
  const mappings: [string, string][] = [
    ["/api/status", "/status"],
    ["/api/sessions", "/sessions"],
    ["/api/cron/jobs", "/cron/jobs"],
    ["/api/model/options", "/models"],
    ["/api/model/info", "/status"],
    ["/api/config", "/config"],
    ["/api/system/stats", "/status"],
    ["/api/skills", "/tools"],
    ["/api/env", "/status"],
    ["/api/gateway", "/status"],
    ["/api/analytics", "/status"],
    ["/api/messaging/platforms", "/status"],
    ["/api/ws", "/events"],
  ];
  ...
  // Strip /api/ prefix for any unmapped endpoints
  if (url.startsWith("/api/")) return "/" + url.slice(5);
  return url;
}

function setSessionHeader(_headers: Headers, _token: string): void {
  // mya fork: no-op — cookie-based auth
}
...
export async function fetchJSON<T>(
  ...
  url = mapMyaUrl(withManagementProfile(url));
  // mya fork: cookie-based auth — no token header needed.
  // The gateway's HttpOnly mya_ws cookie is sent automatically via credentials: "include".
  ...
  if (res.status === 401) {
    // mya fork: cookie-based auth. If we get 401, just throw —
    // the gateway sets the cookie on GET / and it should persist.
