 ...
 *  Returns the status string, or "" on timeout. "done" requires real LLM creds;
 *  "failed" still proves the spawn → gateway → status-report pipeline works. */
async function pollTreeForTerminal(
  port: number,
  sessionId: string,
  timeoutMs = 90_000,
): Promise<string> {
  ...
  while (Date.now() < deadline) {
    try {
      ...
      if (r.ok) {
        const tree = (await r.json()) as Array<{ sessionId: string; status?: string }>;
        const found = tree.find((n) => n.sessionId === sessionId);
        if (found?.status === "done" || found?.status === "failed") return found.status;
      }
    } catch {
      /* transient */
    }
...
describe.skipIf(!MYA_BIN)("[system] role-subagent E2E — spawn → done → /pool/tree", () => {
  ...
  it("a spawned role-subagent reports done + appears nested in /pool/tree", async (ctx) => {
    if (!hasRole) { ctx.skip(); return; } // bundle stale — --role absent; rebuild dist/mya.js (npm run dist)

    const home = mkdtempSync(join(tmpdir(), "mya-e2e-"));
    // Copy real auth.json (if present) so the subagent has LLM creds → "done".
    // Absent creds → "failed" (still proves the status-report pipeline).
    ...
    try {
      serve = spawn(MYA_BIN!, ["serve", "--port", String(port)], {
        // Dev auth bypass (documented) — the test has no dashboard cookie/token.
      ...
      const status = await pollTreeForTerminal(port, sessionId);
