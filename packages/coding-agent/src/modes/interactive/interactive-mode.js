...
import { getProviders, } from "@my-agent/pi-ai/compat";
import { CombinedAutocompleteProvider, Container, fuzzyFilter, getCapabilities, hyperlink, Markdown, matchesKey, ProcessTerminal, Spacer, setKeybindings, Text, TruncatedText, TUI, visibleWidth, } from "@my-agent/tui";
import chalk from "chalk";
import { spawn, spawnSync } from "child_process";
import { APP_NAME, APP_TITLE, CONFIG_DIR_NAME, getAgentDir, getAuthPath, getDebugLogPath, getDocsPath, getShareViewerUrl, VERSION, } from "../../config.ts";
...
import { CACHE_TTL_MS, collectCacheMisses, computeCacheWaste, detectCacheMiss, } from "../../core/cache-stats.ts";
...
import { SessionManager, sessionEntryToContextMessages } from "../../core/session-manager.ts";
...
import { formatAuthSelectorProviderType, OAuthSelectorComponent, } from "./components/oauth-selector.ts";
import { ScopedModelsSelectorComponent } from "./components/scoped-models-selector.ts";
import { SessionSelectorComponent } from "./components/session-selector.ts";
import { SettingsSelectorComponent } from "./components/settings-selector.ts";
import { SkillInvocationMessageComponent } from "./components/skill-invocation-message.ts";
import { BranchSummaryStatusIndicator, CompactionStatusIndicator, IdleStatus, RetryStatusIndicator, WorkingStatusIndicator, } from "./components/status-indicator.ts";
...
import { getAvailableThemes, getAvailableThemesWithPaths, getEditorTheme, getMarkdownTheme, getThemeByName, onThemeChange, setRegisteredThemes, stopThemeWatcher, Theme, theme, } from "./theme/theme.ts";
...
export class InteractiveMode {
    ...
    async init() {
        ...
        if (this.options.verbose || !this.settingsManager.getQuietStartup()) {
            ...
            const onboarding = theme.fg("dim", `mya can explain its own features and look up its docs. Ask it how to use or extend mya.`);
    ...
    uncaughtCrash(error) {
        ...
        console.error("mya exiting due to uncaughtException:");
        console.error(error);
        process.exit(1);
    }
    /**
        ...
        this.showSelector((done) => {
            const selector = new TrustSelectorComponent({
                ...
                onSelect: (selection) => {
                    ...
                    this.showStatus(`Saved trust decision: ${selection.trusted ? "trusted" : "untrusted"}. Restart mya for this to take effect.`);
                },
                onCancel: () => {
                    done();
                    this.ui.requestRender();
        ...
        if (subs.length === 0) {
            ...
            for (const s of subs) {
                ...
                if (s.allowedTools?.length)
                    out += `   ${theme.fg("dim", "Tools:")} ${s.allowedTools.join(", ")}\n`;
                if (s.output)
                    out += `   ${theme.fg("dim", "Output:")} ${s.output.slice(0, 80).replace(/\n/g, " ")}${s.output.length > 80 ? "..." : ""}\n`;
                if (s.error)
                    out += `   ${theme.fg("red", "Error:")} ${s.error}\n`;
                out += "\n";
            }
        }
