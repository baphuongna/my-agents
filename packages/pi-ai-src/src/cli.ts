...
async function main(): Promise<void> {
	...
	if (!command || command === "help" || command === "--help" || command === "-h") {
		...
		console.log(`Usage: npx @my-agent/ai <command> [provider]
...
Examples:
  npx @my-agent/ai login              # interactive provider selection
  npx @my-agent/ai login anthropic    # login to specific provider
  npx @my-agent/ai list               # list providers
`);
		return;
	}
	...
	if (command === "login") {
		...
		if (!PROVIDERS.some((p) => p.id === provider)) {
			...
			console.error(`Use 'npx @my-agent/ai list' to see available providers`);
			process.exit(1);
		}
	...
	console.error(`Use 'npx @my-agent/ai --help' for usage`);
	process.exit(1);
}
